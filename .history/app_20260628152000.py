...
# Simple i18n: map Spanish source strings to English translations.
EN_TRANSLATIONS = {
    ...
    'Seleccione el mes de interes': 'Select the month of interest',
    'Calcular intervalos con Strike Price fijo': 'Calculate intervals with fixed Strike Price',
    'Ingrese el Strike Price': 'Enter the Strike Price',
    'Graficar': 'Plot',
    ...
    'Noviembre': 'November',
    'Diciembre': 'December',
    'A continuacion se presentan los intervalos de confianza con un Strike Price fijo de 8': 'Below are the confidence intervals with a fixed Strike Price of 8',
...
# Make the translate function available inside Jinja templates as `_`
app.jinja_env.globals.update(_=_translate)
@app.context_processor
def inject_language():
    return dict(current_language=get_locale())

# Expose `_` in Python code for convenience
_ = _translate
...
def intervalos(lugar, mes):
    ...
    filtro = (df['Region'] == lugar) & (df['Month'] == mes) & (df['Variable'] == 'Call_HDD')
    
    # Obtener el valor de 'Confidence Interval' para los datos filtrados
    resultado = df.loc[filtro, 'Confidence Interval']
...
def intervalos(lugar, mes):
    ...
    Call_Hdd_str = str(Call_Hdd[0]) if len(Call_Hdd) > 0 else "No disponible"
    Put_HDD_str = str(Put_HDD[0]) if len(Put_HDD) > 0 else "No disponible"
    Call_CDD_str = str(Call_CDD[0]) if len(Call_CDD) > 0 else "No disponible"
    Put_CDD_str = str(Put_CDD[0]) if len(Put_CDD) > 0 else "No disponible"
...
def calcular_intervalos():
    data = request.json
    ciudad = data['ciudad']
    mes = int(data['mes'])  # Asegúrate de convertir el mes a entero

    # Llama a la función intervalos con los parámetros recibidos
    Call_Hdd_str, Put_HDD_str, Call_CDD_str, Put_CDD_str = intervalos(ciudad, mes)

    # Devuelve los intervalos como JSON
    return jsonify({
